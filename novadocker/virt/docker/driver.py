...
class DockerDriver(driver.ComputeDriver):
    ...
    def _pull_missing_image(self, context, image_meta, instance):
        ...
        # Chris and Ian looked into doing this by passing around file handles
        # but we found that there are no methods available that let us get
        # a file handle for either call.  We could use threads and a pipe here
        # but that seems overly complex.
        # What we really need is image_service.download_fh() that returns a
        # file handle that we can pass into docker load_repository().
        # Currently image_service.download() will try to write to the fh
        # if you pass it in.
