...
class DockerDriver(driver.ComputeDriver):
    ...
    def _pull_missing_image(self, context, image_meta, instance):
        ...
        with utils.tempdir(dir=snapshot_directory) as tmpdir:
            try:
                ...
                LOG.error(_('Cannot load repository file: %s'),
                          e, instance=instance)
    ...
    def _start_container(self, container_id, instance, network_info=None):
        ...
        try:
            ...
            LOG.error(_('Cannot setup network: %s'),
                        e, instance=instance)
    ...
    def snapshot(self, context, instance, image_href, update_task_state):
        ...
        if ':' not in image['name']:
            commit_name = unicode(image['name']).encode('utf-8')
            tag = 'latest'
        else:
            parts = unicode(image['name']).encode('utf-8').rsplit(':', 1)
            commit_name = parts[0]
            tag = parts[1]
        ...
        try:
            ...
            content = raw.read()
            image_service.update(context, image_href, metadata, content)
        except Exception as e:
            LOG.error(_('Error saving image: %s'),
                      e, instance=instance)
