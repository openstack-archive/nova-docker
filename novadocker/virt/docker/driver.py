...
class DockerDriver(driver.ComputeDriver):
    ...
    def _get_registry_ip(self):
        ...
        if not registry:
            return default_ip
        # NOTE(samalba/paulczar): The registry service always binds on port 5000 in the
        # container, we can use this to get the host IP.  If the host IP is
        # 0.0.0.0 then we assume it's the local IP of the nova box???
        try:
            registry_ip = container['NetworkSettings']['Ports']['5000/tcp'][0]['HostIp']
