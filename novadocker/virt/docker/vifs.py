...
class DockerGenericVIFDriver(object):
    ...
    def plug_midonet(self, instance, vif):
        """Plug into MidoNet's network port by binding the vif to a MidoNet
        virtual port"""
        if_local_name = 'tap%s' % vif['id'][:11]
        if_remote_name = 'ns%s' % vif['id'][:11]
        bridge = vif['network']['bridge']
        ...
        try:
            ...
            linux_net.create_ovs_vif_port(bridge, if_local_name,
                                          ...
                                          vif['address'],
                                          instance['uuid'])
            undo_mgr.undo_with(lambda: linux_net.delete_ovs_vifg_port(
                bridge, if_local_name))
            utils.execute('ip', 'link', 'set', if_local_name, 'up',
                          run_as_root=True)
    ...
    def unplug_midonet(self, instance, vif):
        """Unplug the VIF by deleting the port from the bridge and unbinding
        the port from the MidoNet virtual port"""
        try:
            linux_net.delete_ovs_vif_port(vif['network']['bridge'],
                                          vif['devname'])
