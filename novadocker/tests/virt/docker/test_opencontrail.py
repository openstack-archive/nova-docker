    ...
    def test_attach_vrouter(self, mock_find_by_uuid, mock_find_pid):
        ...
        calls = [
            ...
            mock.call('ip', 'link', 'set', if_remote_name, 'netns', 'my_vm',
                      run_as_root=True),
            mock.call('vrouter-port-control', '--oper=add --uuid=%s'
                      ' --instance_uuid=%s --vn_uuid=%s --vm_project_uuid=%s'
                      ' --ip_address=%s --ipv6_address=None --vm_name=%s'
                      ' --mac=%s --tap_name=%s --port_type=NovaVMPort'
                      ' --tx_vlan_id=-1 --rx_vlan_id=-1' % (
                          vid, instance['uuid'], vnid, instance['project_id'],
                          fixed_ip_addr, instance['display_name'], address,
                          if_local_name),
                      run_as_root=True),
