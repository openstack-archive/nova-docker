...
if is_ubuntu; then
  ...
  sudo find /var/log/ -name "docker*" -print -exec sudo cp {} $INSTALLDIR/logs/ \;
elif is_fedora; then
...
sudo find /var/lib/docker/containers/ -name "*.log" -print -exec sudo cp {} $INSTALLDIR/logs/containers/ \;